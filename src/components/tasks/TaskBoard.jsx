    ...
    <div className="h-full flex flex-col gap-4 p-2">
      ...
      <div className="flex gap-3 overflow-x-auto pb-4 flex-1 items-start">
        {COLUMNS.map(status => {
          ...
          return (
            <div key={status} className="min-w-[210px] w-[210px] flex flex-col gap-2 shrink-0">
              <div className="flex items-center gap-2 px-1 mb-1">
                <span className={`text-[11px] font-semibold uppercase tracking-wider ${COLORS[status]}`}>
                  {LABELS[status]}
                </span>
              ...
              <div className="flex flex-col gap-2">
                {col.length === 0 && (
                  <div className="border border-dashed border-white/5 rounded-lg h-16 flex items-center justify-center">
                    <span className="text-[11px] text-gray-700">empty</span>
                  </div>
                )}
                {col.map(task => (
                  <TaskCard
